...
from ck_g_cnn.nn.kernels.Sim2_kernel import Sim2GroupKernelGSeparable2D

from ck_g_cnn.groups import Sim2


class GroupConvGSeparable2D(ContinuousKernelGroupConv):
    ...
    def __init__(self, group, kernel_size, in_channels, out_channels, num_group_elem, ck_net_num_hidden,
        ...
        """ Create a group convolution layer, performing a convolution on (a homogeneous space of) the groups.
         Let i,j be in and output channels, this implementation defines the group convolution kernel as:
            ...
            k_{ij}(x, g1, g2) = k_{ij}(g1) k_{ij}(g2) k_{ij}(x)
        ...
        if type(group) == Sim2:
            self.kernel = Sim2GroupKernelGSeparable2D(
                group=group,
                kernel_size=kernel_size,
                in_channels=in_channels,
                out_channels=out_channels,
    ...
    def forward(self, x, grid_H, sampled_group_elements=None):
        ...
        # reshape x into [batch_dim, channels, rot_dim, scale_dim, spatial_1, spatial_2], expanding the group into
        # separate dimensions
        x = x.reshape(-1, x.shape[1], grid_SO2.shape[0], grid_Rplus.shape[0], x.shape[3], x.shape[4])
        ...
        #   [batch, channel_out, rot_out, channel_in, scale_in, width, height]
        x = torch.einsum(
            'bcrswh,ogcr->bogcswh',
            x,
            SO2_kernel.reshape(
        ...
        # [batch, channel_out, rot_out, channel_in, scale_in, width, height] *
        # [channel_out, scale_out, channel_in, scale_in] ->
        #   [batch, channel_out, rot_out, scale_out, channel_in, width, height] ->
        #   [batch, channel_out * rot_out * scale_out * channel_in, width, height]
        x = torch.einsum(
            'bogcswh,ofcs->bogfcwh',  # sum over input channel dim
            x,
            ...
            -1,
            self.kernel.out_channels * sampled_elem_SO2.shape[0] * sampled_elem_Rplus.shape[0] * self.kernel.in_channels,
            x.shape[-2],
            x.shape[-1]
        )
        ...
        x = F.conv2d(
            ...
            stride=self.stride,
            groups=(self.kernel.out_channels * sampled_elem_SO2.shape[0] * sampled_elem_Rplus.shape[0])  # each out channel and elem gets own weight
        )
