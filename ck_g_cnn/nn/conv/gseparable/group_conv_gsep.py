...
from ck_g_cnn.nn.kernels.SE2_kernel import SE2GroupKernelGSeparable
from ck_g_cnn.nn.kernels.R2xRplus_kernel import R2xRplusGroupKernelGSeparable
from ck_g_cnn.nn.kernels.Sim2_kernel import Sim2GroupKernelGSeparable

from ck_g_cnn.groups import SE2, Rplus, Sim2


class GroupConvGSeparable(ContinuousKernelGroupConv):
    ...
    def __init__(self, group, kernel_size, in_channels, out_channels, num_group_elem, ck_net_num_hidden,
        ...
        if type(group) == SE2:
            self.kernel = SE2GroupKernelGSeparable(
                group=group,
                kernel_size=kernel_size,
                in_channels=in_channels,
                out_channels=out_channels,
            ...
            self.kernel = R2xRplusGroupKernelGSeparable(
                group=group,
                kernel_size=kernel_size,
                in_channels=in_channels,
                out_channels=out_channels,
            ...
            self.kernel = Sim2GroupKernelGSeparable(
                group=group,
                kernel_size=kernel_size,
                in_channels=in_channels,
                out_channels=out_channels,
    ...
    def forward(self, x, grid_H, sampled_group_elements=None):
        ...
        # combine H and Rn conv kernels, turns out this is faster than a sequential operation
        Rn_conv_kernel = Rn_conv_kernel.unsqueeze(-3).repeat(1, 1, 1, grid_H.shape[0], 1, 1)
        H_conv_kernel = H_conv_kernel.repeat(1, 1, 1, 1, self.kernel.kernel_size, self.kernel.kernel_size)
        conv_kernels = Rn_conv_kernel * H_conv_kernel
